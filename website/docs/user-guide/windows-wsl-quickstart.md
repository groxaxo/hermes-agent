...
One exception: **MCP bridges that launch Windows binaries.** If you're using a Windows-side Chrome MCP bridge from WSL (see [MCP guide: WSL → Windows Chrome](/docs/guides/use-mcp-with-hermes#wsl2-hermes-to-windows-chrome)), Windows may complain with a `UNC` warning if Hermes's current working directory is `~`. In that case, start Hermes from somewhere under `/mnt/c/` so the Windows process has a drive-letter cwd.
...
- **[MCP guide → WSL → Windows Chrome](/docs/guides/use-mcp-with-hermes#wsl2-hermes-to-windows-chrome)** — controlling your signed-in Windows Chrome from Hermes in WSL.
